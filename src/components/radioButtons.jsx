            ...
            <div className='flex flex-wrap w-full items-center gap-4'>
                {options.map(option => (
                    <div className='inline-flex rounded-xl justify-center items-center' key={option?.value + '_' + name}>
                        <input type='radio' name={name} id={option?.value + '_' + name} value={option?.value} checked={state[name] === option?.value || option?.checked} onChange={(e) => handleRadioChange(e, option?.onChange || function(){})} required={required ? 'required' : ''} className='opacity-0 absolute' />
