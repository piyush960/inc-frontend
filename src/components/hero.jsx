...
function Hero() {
    ...
    useEffect(() => {
        const moveShadow = (e, title) => {
            ...
            title.style.textShadow = '-' + rYP / (factor * 10) + 'px ' + rXP / (factor * 80) + 'px 10px rgba(126, 188, 242, 0.2), ' + rYP / (factor * 10) + 'px -' + rXP / (factor * 60) + 'px 10px rgba(242, 163, 15, 0.2), ' + rXP / (factor * 70) + 'px ' + rYP / (factor * 12) + 'px 10px rgba(162, 205, 242, 0.2)';
        }
    ...
    return (
        <div className='hero'>
            ...
            <div className='hero-title'>
                <h4 className='hero-lines'>SOCIETY FOR COMPUTER TECHNOLOGY AND RESEARCH'S</h4>
                <h3 className='hero-lines'>PUNE INSTITUTE OF COMPUTER TECHNOLOGY PRESENTS</h3>
                <div className='inc-title' ref={titleRef}>
                    <h1>IMPETUS AND CONCEPTS</h1>
                </div>
            </div>
            <div className='hero-cube'>
                <Cube className='scale-150' />
            </div>
        </div>
    );
}
