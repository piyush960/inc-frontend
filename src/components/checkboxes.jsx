...
function Checkboxes({ label, options, name, state, setState, error }) {
    const handleCheckboxChange = useCallback((e) => {
        const { value } = e.target
        setState(prevState => {
            let newState = { ...prevState }
            if (!newState.domains.includes(value)) {
                newState.domains = [...newState.domains, value] || []
            } else {
                newState.domains = newState.domains.filter(item => item !== value)
            }
            return newState
        })
    }, [setState, state])
