...
function JudgeAllocation() {
    function handleButtonClick() {
        const url = `https://pictinc.org/judge/result-concepts?jid=${judgesData.jid}&pid=${judgesData.pid}`;
            ...
            <div className="py-8 flex flex-col items-center mx-5">
                {judgesData.map((judge) => (
                    <div key={judge.id} className="rounded-lg outline-dashed outline-2 outline-offset-[3px] my-2 outline-light_blue px-4 py-2 bg-faint_blue/10 mb-3 w-[90%] md:w-[80%]">
                        ...
                        <button className="px-2 md:px-6 py-4 text-white font-semibold border border-transparent focus:outline-0 rounded-xl bg-faint_blue/30 transition-all duration-300 hover:text-gold hover:border-light_blue hover:bg-faint_blue/10"
                         onClick={handleButtonClick}>Result</button>
                    </div>
                ))}
            </div>
